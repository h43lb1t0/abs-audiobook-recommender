        ...
        try:
            ...
            if new_count > 0:
                try:
                    ...
                    new_log = BackgroundCheckLog(checked_new_books_at=current_time)
                    db.session.add(new_log)
                    db.session.commit()
                    logger.info(
                        f"Scheduled library indexing complete. New items: {new_count}. Logged to DB."
                    )
...
def scheduled_user_activity_check(app, socketio_instance):
    ...
    with app.app_context():
        ...
        try:
            ...
            last_check_log = BackgroundCheckLog.query.order_by(
                BackgroundCheckLog.id.desc()
            ).first()
            ...
            if last_check_log:
                ...
                if last_check_log.checked_new_books_at:
                    checked_at = datetime.fromisoformat(
                        last_check_log.checked_new_books_at
                    )
                    ...
                    if (
                        checked_at < time_threshold
                        and not last_check_log.created_recommendations
                    ):
                        logger.info(
                            "Found pending new books check. Triggering global recommendation update."
                        )
            ...
            for user in users:
                ...
                if not should_generate:
                    last_rec = UserRecommendations.query.filter_by(
                        user_id=user_id
                    ).first()
                    ...
                    if recent_changes:
                        logger.info(
                            f"User {user.username} has recent activity since {last_rec_time}. triggering recommendations."
                        )
                        should_generate = True

                if should_generate:
                    logger.info(
                        f"BGT: Generating background recommendations for {user.username} (ID: {user_id})"
                    )
                    try:
                        ...
                        with app.test_request_context(f"/?lang={lang}"):
                            ...
                            db.session.commit()
                            logger.info(
                                f"BGT: Recommendations updated for {user.username}. Emitting to room {user_id}"
                            )

                            # Broadcast websocket event to notify user of new recommendations
                            try:
                                logger.info(
                                    f"BGT: Attempting emit to {user_id} with socketio {socketio_instance}"
                                )
                                ...
                                logger.info(
                                    f"BGT: Successfully emitted recommendations_ready event for {user.username} to room {user_id}"
                                )
                            except Exception as ws_error:
                                logger.error(
                                    f"BGT: Failed to broadcast websocket event: {ws_error}"
                                )

                    except Exception as e:
                        logger.error(
                            f"BGT: Error generating recommendations for {user.username}: {e}"
                        )
                else:
                    logger.info(
                        f"BGT: No recommendations needed for {user.username}. changes={bool(recent_changes) if 'recent_changes' in locals() else 'N/A'}, global={new_books_trigger}"
                    )
